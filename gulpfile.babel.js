...
export const sw = (done) => {
	...
	if ( production && config.site.serviceworker ) {
		...
		return workbox.generateSW({
								   ...
								   offlineGoogleAnalytics: config.tracking.ga.offline,
...
export const vendorsjs = () => {
	...
	return gulp.src( config.js.vendors.files.map( function(base) { return paths.source + paths.vendorsJS + base } ), {allowEmpty: true} )
			   ...
			   .pipe(_if( !production && config.compiler.rememberFiles, changed(paths.build + paths.scripts) ))
			   .pipe(removeCode({removeUnusedCode: config.compiler.removeUnusedCode}))
...
export const minify = (done) => {
	...
	gulp.src( paths.build + '**/*.js', { allowEmpty: true } )
		...
		.pipe(_if( production && config.compiler.transpile, babel({
			presets: [config.compiler.transpiler],
			compact: false
		})) )
...
export const preview = (done) => {
	...
	if ( config.compiler.previewURL ) {
		...
		gulp.src( paths.build + config.compiler.previewURL )
			.pipe(open());
...
// REFRESH (USED BY BROWSERSYNC ONLY)
...
// PARTIAL TESTING
gulp.task('dev', gulp.series(version, clear, revconfig, html, injectwebfontloader, lintjs, libsjs, frameworkjs, vendorsjs, appjs, combinevendorsjs, combineappjs, css, tempcss, criticalcss, move, appicons, meta, a11ycheck, fingerprintassets, injectscripts, optimizeassets, clean, minify, completed));
